...
class WatchDog:
    ...
    def check_angular_stuck(self, dataframe: DataFrame):
        ...
        while self.collector.left_pointer < self.collector.right_pointer:
            actual = (dataframe.loc[self.collector._df["row"] == self.collector.left_pointer]["angular_velocity"]).iloc[
                0
            ]
            commanded = (dataframe.loc[self.collector._df["row"] == self.collector.left_pointer]["cmd_vel_twist"]).iloc[
                0
            ]
    ...
    def check_linear_stuck(self, dataframe: DataFrame):
        ...
        self.collector.left_pointer = self.collector.right_pointer - DF_THRESHOLD
        while self.collector.left_pointer < self.collector.right_pointer:
            actual = (dataframe.loc[self.collector._df["row"] == self.collector.left_pointer]["linear_velocity"]).iloc[
                0
            ]
