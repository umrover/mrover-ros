...
class WatchDog:
    ...
    def get_start_end_rotations(self, dataframe: DataFrame) -> Tuple[SO3, SO3]:
        start_rot = np.array(
            [dataframe["rot_x"].iloc[0], dataframe["rot_y"].iloc[0], dataframe["rot_z"].iloc[0], dataframe["rot_w"].iloc[0]]
        )
        end_rot = np.array(
            [dataframe["rot_x"].iloc[-1], dataframe["rot_y"].iloc[-1], dataframe["rot_z"].iloc[-1], dataframe["rot_w"].iloc[-1]]
        )
    ...
    def check_angular_stuck(self, delta_time, delta_rot, dataframe) -> bool:
        ...
        turn_are_all_same_sign = dataframe["cmd_vel_twist"].apply(np.sign).eq(np.sign(dataframe["cmd_vel_twist"].iloc[0])).all()
    ...
    def check_linear_stuck(self, delta_time, delta_pos, dataframe) -> bool:
        ...
        linear_are_all_same_sign = dataframe["cmd_vel_x"].apply(np.sign).eq(np.sign(dataframe["cmd_vel_x"].iloc[0])).all()
