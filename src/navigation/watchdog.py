...
class WatchDog:
    ...
    def is_stuck(self):
       # make sure timing is ok with pointers
        if len(self.collector._df.index) > DF_THRESHOLD:
    ...
    def check_angular_stuck(self):
        ...
        while (self.collector.left_pointer < self.collector.right_pointer):
            ...
            commanded = (self.collector._df.loc[self.collector._df['row'] == self.collector.left_pointer]['commanded_angular_vel']).iloc[0]
    ...
    def check_linear_stuck(self):
        ...
        while (self.collector.left_pointer < self.collector.right_pointer):
            ...
            commanded = (self.collector._df.loc[self.collector._df['row'] == self.collector.left_pointer]['commanded_linear_vel']).iloc[0]
