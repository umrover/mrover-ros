...
class GPS_Correction:
    ...
    def get_heading_change(self, msg: Twist):
        ...
        """
        new_vel = np.array([[msg.linear.x, msg.linear.y, msg.linear.z], [msg.angular.x, msg.angular.y, msg.angular.z]])
    ...
    def velocity_callback(self, msg: Twist):
        if self.get_heading_change(msg):
            if (
                self.gps_points.size > self.num_points_threshold
            ):  # If we have enough points update heading before deleting all points
        ...
        while self.driving_straight and (not self.kill_flag):
            try:
                ...
                self.gps_points.append(
                    transform.position, axis=0
                )  # TODO: might have to reshape transform.position to (1,3)
                ...
                if self.gps_points.size == (self.num_points_threshold // 2):
                    self.transform_to_update = SE3.from_tf_tree(
                        self.tf_buffer, parent_frame=self.world_frame, child_frame=self.rover_frame
                    )
            ...
            if (
                self.gps_points.size() > self.num_points_threshold
            ):  # compute the heading correction if we cross an upper points threshold
    ...
    def get_heading_correction(self):
        ...
        heading_rotation = np.array([[heading[0], -heading[1], 0], [heading[1], heading[0], 0], [0, 0, 1]])
