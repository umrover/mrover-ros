...
def check_failure_zone(fz, start, end, res):
    ...
    assert(fz.intersect(start, end) == res)

fz = FailureZone(np.array([[0, 0], [0, 1], [1, 1], [1, 0]]))
...
check_failure_zone(fz, [0.2, 0.2], [0.2, 0.2], False) # no intersection degenerate path
...
fz = FailureZone(np.array([[0, 1], [1, 2], [2, 1], [1, 0]])) # rotated fz 
