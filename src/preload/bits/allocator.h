// Note(quintin):
// This is copied from /usr/include/c++/13/bits/allocator.h
// C++20 removed some features from the allocator class
// They are disabled via ifdefs, which I removed, enabling them
// This is because boost uses them since it is really old
// C++ should have never broke this backwards compatibility!
// How silly!

#if defined(__linux__) && defined(__GNUC__) && (__GNUC__ == 13)
...
namespace std _GLIBCXX_VISIBILITY(default) {
   ...
   */
    ...
    public:
        ...
        __attribute__((__always_inline__)) constexpr allocator(allocator<_Up> const&) noexcept {}
   ...
   */
    ...
    public:
        ...
        __attribute__((__always_inline__))
        _GLIBCXX20_CONSTEXPR
        allocator(allocator const& __a) _GLIBCXX_NOTHROW
        ...
        __attribute__((__always_inline__))
        _GLIBCXX20_CONSTEXPR
        allocator(allocator<_Tp1> const&) _GLIBCXX_NOTHROW {}
        ...
        operator==(allocator const&, allocator const&) _GLIBCXX_NOTHROW { return true; }

#if __cpp_impl_three_way_comparison < 201907L
        friend __attribute__((__always_inline__)) _GLIBCXX20_CONSTEXPR bool
        operator!=(allocator const&, allocator const&) _GLIBCXX_NOTHROW { return false; }
#endif
   ...
   */
    ...
    operator==(allocator<_T1> const&, allocator<_T2> const&)
    ...
    template<typename _Tp>
    class allocator<_Tp const> {
    public:
        typedef _Tp value_type;
        template<typename _Up>
        allocator(allocator<_Up> const&) {}
    };

    template<typename _Tp>
    class allocator<_Tp volatile> {
    public:
        typedef _Tp value_type;
        template<typename _Up>
        allocator(allocator<_Up> const&) {}
    ...
    template<typename _Tp>
    class allocator<_Tp const volatile> {
    public:
        typedef _Tp value_type;
        template<typename _Up>
        allocator(allocator<_Up> const&) {}
