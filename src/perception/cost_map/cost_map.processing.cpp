...
namespace mrover {
    ...
    void CostMapNodelet::occupancyGridCallback(nav_msgs::OccupancyGrid const& msg) {
        ...
        cell_width = static_cast<int>(msg.info.width); //hard set? don't want changing cell dims
        cell_height = static_cast<int>(msg.info.width);
        ...
        auto resolution_global = static_cast<float>(globalGridMsg.info.resolution); //defined in cost_map.hpp?
