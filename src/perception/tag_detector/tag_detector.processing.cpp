...
namespace mrover {
    ...
    std::optional<SE3d> TagDetectorNodelet::getTagInCamFromPixel(sensor_msgs::PointCloud2ConstPtr const& cloudPtr, size_t u, size_t v) {
        ...
        return std::make_optional<SE3d>(Eigen::Vector3d{point.x, point.y, point.z}, Eigen::Quaterniond{1, 0, 0, 0});
    }
    ...
    void TagDetectorNodelet::pointCloudCallback(sensor_msgs::PointCloud2ConstPtr const& msg) {
        ...
        for (size_t i = 0; i < mImmediateIds.size(); ++i) {
            ...
            if (tag.tagInCam) {
                ...
                pushSE3ToTfTree(mTfBroadcaster, immediateFrameId, mCameraFrameId, tag.tagInCam.value());
            }
        }
        ...
        for (auto const& [id, tag]: mTags) {
            if (tag.hitCount >= mMinHitCountBeforePublish && tag.tagInCam) {
                try {
                    ...
                    SE3d tagInParent = SE3fromTfTree(mTfBuffer, parentFrameId, immediateFrameId);
                    pushSE3ToTfTree(mTfBroadcaster, "fiducial" + std::to_string(id), parentFrameId, tagInParent);
