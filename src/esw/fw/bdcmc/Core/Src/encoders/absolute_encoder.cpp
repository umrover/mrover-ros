...
namespace mrover {
    ...
    */
    ...
    AbsoluteEncoderReader::AbsoluteEncoderReader(SMBus<uint8_t, uint16_t> i2c_bus, std::uint8_t A1, std::uint8_t A2, Ratio multiplier, TIM_HandleTypeDef* elapsed_timer)
    ...
    [[nodiscard]] auto AbsoluteEncoderReader::read() -> std::optional<EncoderReading> {
        if (std::optional<std::uint64_t> count = try_read_buffer()) {
            ...
            m_velocity = (m_position - m_position_prev) / elapsed_time;
