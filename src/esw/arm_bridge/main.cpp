...
std::unique_ptr<CanDevice> armLaserCanDevice;
...
bool armLaserCallback(std_srvs::SetBool::Request& req, std_srvs::SetBool::Response& res) {
    armLaserCanDevice->publish_message(InBoundPDLBMessage{ArmLaserCommand{.enable = req.data}});
    res.success = true;
    res.message = "DONE";
    return true;
}
...
int main(int argc, char** argv) {
    ...
    armLaserCanDevice = std::make_unique<CanDevice>(nh, "jetson", "pdlb");
